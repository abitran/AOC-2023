...
int main(int argc, char **argv){
  ...
  for (int i = 0; i < lines; i++)
    sumpowers += powercubes(&Game[i], Game[i].nsets);

  printf("The sum of the powers is: %d\n", sumpowers);
...
int powercubes(game *G, int nsets){

  int maxred, maxgreen, maxblue;

  maxred = G->sets[0].rcubes;
  maxgreen = G->sets[0].gcubes;
  maxblue = G->sets[0].bcubes;

  for (int i = 0; i < nsets; i++){
    if (G->sets[i].rcubes > maxred)
      maxred = G->sets[i].rcubes;
    if (G->sets[i].gcubes > maxgreen)
      maxgreen = G->sets[i].gcubes;
    if (G->sets[i].bcubes > maxblue)
      maxblue = G->sets[i].bcubes;
  }
